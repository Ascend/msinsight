...
class TestDbExport(unittest.TestCase):
    def setUp(self):
        self.temp_dir = tempfile.TemporaryDirectory()  # pylint: disable=consider-using-with
        self.addCleanup(self.temp_dir.cleanup)
        self.output_path = os.path.join(self.temp_dir.name, "ftrace_data.db")
        ...
        self.sample_data = [
            ...
            {"ph": "X", "pid": "Unknown", "tid": "unknown_tid", "ts": 3.0, "dur": 1.0, "name": "Unknown Task"},
    ...
    def test_export_success(self):
        ...
        with contextlib.closing(sqlite3.connect(self.output_path)) as conn:
            ...
            # The invalid_ts event will be skipped entirely due to ValueError
            self.assertEqual(len(slices), 2)

            # First slice: CPU Task (ts=1.5 -> 1500, dur=0.5 -> 500)
            self.assertEqual(slices[0][0], 1500)
            self.assertEqual(slices[0][1], 500)
            self.assertEqual(slices[0][2], "CPU Task")
            self.assertEqual(slices[0][3], '{"irq": "12"}')
            self.assertEqual(slices[0][4], 2000)

            # Second slice: Process Task (ts=2.0 -> 2000, dur=1.5 -> 1500)
            self.assertEqual(slices[1][0], 2000)
            self.assertEqual(slices[1][1], 1500)
            self.assertEqual(slices[1][2], "Process Task")
            self.assertIsNone(slices[1][3])  # empty args {} becomes NULL
            self.assertEqual(slices[1][4], 3500)
