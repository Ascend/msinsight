...
struct MEM_SCOPE_DUMP_EVENT_TYPE {
    ...
    inline const static std::string MALLOC_FREE_HOST = "HOST"; // Host内存申请；新版 Attr 仅 used + process_used
    inline const static std::string MALLOC_FREE_HOST_PINNED =
        "HOST_PINNED"; // 旧版Host锁页类型；导入仍接受，解析时刷为HOST且 Attr.pinned=true
    // 内存访问事件 子类型
    ...
    inline const static std::string SYSTEM_START_TRACE = "START_TRACE"; // trace开始
    inline const static std::string SYSTEM_STOP_TRACE = "STOP_TRACE"; // trace结束
};
// 内存事件 - 类型映射表
const std::unordered_map<std::string, std::set<std::string>> EVENT_TYPE_MAP = {
    {MEM_SCOPE_DUMP_EVENT::MALLOC,
        {MEM_SCOPE_DUMP_EVENT_TYPE::MALLOC_FREE_PTA, MEM_SCOPE_DUMP_EVENT_TYPE::MALLOC_FREE_MINDSPORE,
            ...
            MEM_SCOPE_DUMP_EVENT_TYPE::MALLOC_FREE_PTA_WORKSPACE, MEM_SCOPE_DUMP_EVENT_TYPE::MALLOC_FREE_HOST,
            MEM_SCOPE_DUMP_EVENT_TYPE::MALLOC_FREE_HOST_PINNED}},
