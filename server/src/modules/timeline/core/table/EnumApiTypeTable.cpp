...
void EnumApiTypeTable::SetId(EnumApiTypePO &enumApiTypePO, const std::unique_ptr<SqliteResultSet> &resultSet) {
    enumApiTypePO.id = resultSet->GetUint64(EnumApiTypeColumn::ID);
}

void EnumApiTypeTable::SetName(EnumApiTypePO &enumApiTypePO, const std::unique_ptr<SqliteResultSet> &resultSet) {
    enumApiTypePO.name = resultSet->GetString(EnumApiTypeColumn::NAME);
}

std::unordered_map<uint64_t, std::string> EnumApiTypeTable::QueryStrMap(
