...
uint32_t GetPersistedDepthCount(
    ...
    for (const auto &slice : sliceVec) {
        ...
        depthCount = std::max(depthCount, slice.depth + 1);
    }
    return depthCount;
}
...
std::set<std::pair<uint64_t, uint32_t>> SliceAnalyzer::ComputeResultIds(uint64_t startTime, uint64_t endTime,
    ...
    uint32_t maxDepth = GetPersistedDepthCount(sliceDomain, pythonFunctionIds);
...
void SliceAnalyzer::ComputeScreenSliceIds(
    ...
    const std::string sliceCacheKey = SliceCacheManager::BuildSliceLaneCacheKey(sliceQuery.trackId, false);
    ...
    std::set<std::pair<uint64_t, uint32_t>> idPairVec = ComputeResultIds(
        AddTimestampOffset(sliceQuery.startTime, sliceQuery.minTimestamp),
        AddTimestampOffset(sliceQuery.endTime, sliceQuery.minTimestamp), sliceDomainVec, endList, pythonFunctionIds);
...
void SliceAnalyzer::ComputePythonFunctionSliceIds(
    const SliceQuery &sliceQuery, std::set<uint64_t> &ids, uint64_t &maxDepth, std::map<uint64_t, uint32_t> &depthMap) {
    const std::string sliceCacheKey = SliceCacheManager::BuildSliceLaneCacheKey(sliceQuery.trackId, true);
    auto &instance = SliceCacheManager::Instance();
    ...
    if (instance.GetPythonFunctionStatus(pythonFunctionCacheKey) == PYTHON_FUNCTION_STATUS::UNKNOWN) {
        ...
        const uint64_t count =
            pythonFuncRepo == nullptr ? 0 : pythonFuncRepo->QueryPythonFunctionCountByTrackId(sliceQuery);
        instance.SetPythonFunctionStatus(
            pythonFunctionCacheKey, count == 0 ? PYTHON_FUNCTION_STATUS::NOT_EXIST : PYTHON_FUNCTION_STATUS::EXIST);
    }
    std::vector<SliceDomain> sliceDomainVec =
        instance.GetSliceDomainVec(sliceCacheKey, sliceQuery.GetDataSourceId(), sliceQuery);
    SliceQuery slicePagedQuery;
    if (std::empty(sliceDomainVec)) {
        ...
        slicePagedQuery = SliceCacheManager::GetSlicePagedQuery(pythonFunctionQuery);
        ComputePythonFunctionSliceVecByTimeRange(slicePagedQuery, sliceDomainVec);
        instance.UpdateSliceCache(sliceCacheKey, sliceDomainVec, slicePagedQuery);
    }
    ...
    std::vector<DepthHelper> endList;
    std::set<std::pair<uint64_t, uint32_t>> idPairVec =
        ComputeResultIds(AddTimestampOffset(sliceQuery.startTime, sliceQuery.minTimestamp),
            AddTimestampOffset(sliceQuery.endTime, sliceQuery.minTimestamp), sliceDomainVec, endList, {});
    for (const auto &item : idPairVec) {
        ids.emplace(item.first);
        depthMap[item.first] = item.second;
    }
...
void SliceAnalyzer::ComputeSliceDomainVecAndSelfTimeByTimeRange(const SliceQuery &sliceQuery,
    ...
    uint64_t end = AddTimestampOffset(sliceQuery.endTime, sliceQuery.minTimestamp);
    uint64_t start = AddTimestampOffset(sliceQuery.startTime, sliceQuery.minTimestamp);
...
void SliceAnalyzer::ComputeSliceDomainVecByTrackId(const SliceQuery &sliceQuery, std::vector<SliceDomain> &sliceVec) {
    ...
    if (std::empty(sliceVec)) {
        ...
        sliceCacheManager.UpdateSliceCache(std::to_string(sliceQuery.trackId), sliceVec, slicePagedQuery);
    }
}
