...
const std::string QUERY_KERNEL_SQL =
    ...
    " where name = (select id from STRING_IDS where value = ?) and abs(startNs - ?) <= 500"
    " UNION all "
    " select T.ROWID as id, T.streamId as tid, coalesce(compute.name, schedule.name, T.taskType) as name, "
    " 'Ascend Hardware' as pid, T.depth as depth, 'Ascend Hardware' as metaType from TASK T "
    " left join COMPUTE_TASK_INFO compute on compute.globalTaskId = T.globalTaskId"
    " left join COMMUNICATION_SCHEDULE_TASK_INFO schedule on schedule.globalTaskId = T.globalTaskId"
    " where coalesce(compute.name, schedule.name, T.taskType) = "
    " (select id from STRING_IDS where value = ?) and abs(T.startNs - ?) <= 500"
    " UNION all "
    ...
    " where name = (select id from STRING_IDS where value = ?) and abs(startNs - ?) <= 500"
    " UNION all "
    " SELECT pa.ROWID AS id, "
    " CASE WHEN pa.type = 50003 THEN 'python_stack:' || pa.globalTid ELSE 'pytorch' END AS tid, "
    " name, globalTid AS pid, depth, "
    " CASE WHEN pa.type = 50003 THEN 'PYTORCH_API_PYTHON_STACK' ELSE 'PYTORCH_API' END as metaType "
    " from PYTORCH_API pa "
    " where name = (select id from STRING_IDS where value = ?) and abs(startNs - ?) <= 500"
    " UNION ALL "
...
class TraceDatabaseSqlConst {
  ...
  public:
    ...
    static std::string QueryAffinityOptimizerDbSql(
        const std::string &optimizers, const Protocol::KernelDetailsParams &params) {
        std::string timeCondSql;
        if (params.startTime != params.endTime) {
            timeCondSql += " AND py.endNs >= ? AND py.startNs <= ? ";
        }
