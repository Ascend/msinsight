...
class TextTraceDatabase : public VirtualTraceDatabase {
  public:
    ...
    bool QueryHostSlicesByName(
        const std::string &sliceName, const std::string &metaType, std::vector<Protocol::SimpleSlice> &result) override;
    ...
    uint64_t SameOperatorsCount(const std::string &name, const std::vector<uint64_t> &trackIdList, uint64_t &startTime,
        uint64_t &endTime, bool isPythonStack = false);
