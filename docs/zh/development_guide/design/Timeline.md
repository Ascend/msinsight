...
时间线（Timeline）界面包含工具栏（区域一）、时间线树状图（区域二）、图形化窗格（区域三）和数据窗格（区域四）四个部分。
...
**（3）** 调用请求处理 Handler 去处理数据，Handler 会调用 process 层方法去处理（此处之所以增加 process 层，是因为很多数据并不是直接查询完数据库数据即可，而是需要进一步地处理，这些处理，会在 process 这一层实现。），process 处理层会去相应的数据库中查询数据，并对查询完的数据进一步分类、排序、过滤等操作，以得到最终的结果，返回给 Handler。
