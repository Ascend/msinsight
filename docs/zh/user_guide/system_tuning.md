...
<tr>
  ...
  <tr>
    <td class="tg-0pky">NIC</td>
    <td class="tg-0pky">具体泳道信息详见：<br>1. 《DB格式性能数据文件参考》文档中“<a href="https://gitcode.com/Ascend/msprof/blob/26.2.0/docs/zh/user_guide/profile_data_file_references_db.md#nic">NIC</a>”章节“表 1 格式”字段名列；<br>2. 《性能数据文件参考》文档“Text格式性能数据”章节中“<a href="https://gitcode.com/Ascend/msprof/blob/26.2.0/docs/zh/user_guide/profile_data_file_references.md#nic%E6%AF%8F%E4%B8%AA%E6%97%B6%E9%97%B4%E8%8A%82%E7%82%B9%E7%BD%91%E7%BB%9C%E4%BF%A1%E6%81%AF">nic（每个时间节点网络信息）</a>”章节“表 1 字段说明”字段名列和“表 2 字段说明”字段名列。</td>
    <td class="tg-0pky">具体泳道含义详见：<br>1. 《DB格式性能数据文件参考》文档中“<a href="https://gitcode.com/Ascend/msprof/blob/26.2.0/docs/zh/user_guide/profile_data_file_references_db.md#nic">NIC</a>”章节“表 1 格式”含义列；<br>2. 《性能数据文件参考》文档“Text格式性能数据”章节中“<a href="https://gitcode.com/Ascend/msprof/blob/26.2.0/docs/zh/user_guide/profile_data_file_references.md#nic%E6%AF%8F%E4%B8%AA%E6%97%B6%E9%97%B4%E8%8A%82%E7%82%B9%E7%BD%91%E7%BB%9C%E4%BF%A1%E6%81%AF">nic（每个时间节点网络信息）</a>”章节“表 1 字段说明”字段含义列和“表 2 字段说明”字段含义列。</td>
  </tr>
  <tr>
    <td class="tg-0pky">RoCE</td>
    <td class="tg-0pky">具体泳道信息详见：<br>1. 《DB格式性能数据文件参考》文档中“<a href="https://gitcode.com/Ascend/msprof/blob/26.2.0/docs/zh/user_guide/profile_data_file_references_db.md#roce">ROCE</a>”章节“表 1 格式”字段名列<br>2. 《性能数据文件参考》文档“Text格式性能数据”章节中“<a href="https://gitcode.com/Ascend/msprof/blob/26.2.0/docs/zh/user_guide/profile_data_file_references.md#roceroce%E9%80%9A%E4%BF%A1%E6%8E%A5%E5%8F%A3%E5%B8%A6%E5%AE%BD">roce（RoCE通信接口带宽）</a>”章节“表 1 字段说明”字段名列和“表 2 字段说明”字段名列。</td>
    <td class="tg-0pky">具体泳道含义详见：<br>1. 《DB格式性能数据文件参考》文档中“<a href="https://gitcode.com/Ascend/msprof/blob/26.2.0/docs/zh/user_guide/profile_data_file_references_db.md#roce">ROCE</a>”章节“表 1 格式”含义列<br>2. 《性能数据文件参考》文档“Text格式性能数据”章节中“<a href="https://gitcode.com/Ascend/msprof/blob/26.2.0/docs/zh/user_guide/profile_data_file_references.md#roceroce%E9%80%9A%E4%BF%A1%E6%8E%A5%E5%8F%A3%E5%B8%A6%E5%AE%BD">roce（RoCE通信接口带宽）</a>”章节“表 1 字段说明”字段含义列和“表 2 字段说明”字段含义列。</td>
  </tr>
  <tr>
    <td class="tg-0pky">NetDev Stats</td>
    <td class="tg-0pky">具体泳道信息详见《DB格式性能数据文件参考》文档中“<a href="https://gitcode.com/Ascend/msprof/blob/26.2.0/docs/zh/user_guide/profile_data_file_references_db.md#netdev_stats">NETDEV_STATS</a>”章节“表 1 格式”字段名列</td>
    <td class="tg-0pky">具体泳道含义详见《DB格式性能数据文件参考》文档中“<a href="https://gitcode.com/Ascend/msprof/blob/26.2.0/docs/zh/user_guide/profile_data_file_references_db.md#netdev_stats">NETDEV_STATS</a>”章节“表 1 格式”含义列</td>
  </tr>
  <tr>
...
时间线（Timeline）界面支持通过鼠标功能栏调整视图，界面如[图 1 悬浮工具栏](#悬浮工具栏)所示，主要功能如下：
...
- 在图形化窗格中，可使用鼠标右键菜单进行缩放展示，具体功能参见[**表 1**  鼠标右键菜单功能](#鼠标右键菜单功能)。
    ...
    **表 1**  鼠标右键菜单功能<a id="鼠标右键菜单功能"></a>
    ...
    |撤销缩放（0）|Undo Zoom(0)|撤销缩放，括号中的数字会随着缩放次数变化，初始状态为0。|在放大后的时间线（Timeline）界面，单击鼠标右键，弹出菜单；单击撤销缩放，界面缩小一次，括号中的数字会随之减一。|
...
- 单击界面左上方工具栏中的![](./figures/system_tuning/zh-cn_image_0000002500040294.png)，在弹出输入框中输入需要搜索的内容，然后按回车键，则会匹配对应的算子或API，搜索结果匹配算子和API总数，在界面中也会高亮显示匹配的算子或API，如[**图 1**  搜索算子](#搜索算子)所示。
    ...
    单击搜索框前方的下拉菜单可选择查看不同的卡，也可选择“ALL”查看所有卡，系统默认选择第一张卡，如[**图 2** 选卡功能](#选卡功能)。
...
- 手动设置偏移量
    ...
    对于多机多卡场景，由于机器上时间不准，可能造成多卡间时间线（Timeline）相对位置不准确，如[**图 11**  设置偏移量](#设置偏移量)所示，通过以**Host**或者**Device**的维度设置偏移量，可以将对应泳道左右移动，从而达到时间“校准”的目的。偏移量的单位为ns，负值为右移，正值为左移。

    **图 11**  设置偏移量<a id="设置偏移量"></a>

    ![](./figures/system_tuning/single_card_time_adjustment_1.png "设置偏移量")

**支持时间对齐**

为方便数据查看对比更直观，支持多泳道算子的时间对齐，选择想要对齐的泳道算子，单击右键，在功能菜单中选择“**时间对齐**”功能右侧的“**左对齐**”或者“**右对齐**”即可实现。如[**图 1 时间对齐**](#时间对齐)所示。

**图 1**  时间对齐<a id="时间对齐"></a>

![](./figures/system_tuning/timeline_align_1.png "时间对齐")

- 选择“左对齐”后，效果如[**图 2 时间左对齐**](#时间左对齐)所示。

  **图 2**  时间左对齐<a id="时间左对齐"></a>

  ![](./figures/system_tuning/timeline_left_align_1.png "时间左对齐")

- 选择“右对齐”后，效果如[**图 3 时间右对齐**](#时间右对齐)所示。
...
- **算子时间对齐**
    ...
    3. 使用键盘快捷键L（左对齐），将选中的算子与基准算子左边界对齐，如[**图 16** 算子左边界对齐](#算子左边界对齐)所示。
        ...
        **图 17** 算子右边界对齐<a id="算子右边界对齐"></a>
        ...
        > [!NOTE]
        >
        > 无论左对齐还是右对齐，与选中算子为相同类型NPU泳道中的算子也会随之一起偏移。
...
- **支持跳转到对应流**
    ...
    **图 20** 跳转到对应流<a id="跳转到对应流"></a>
    ...
    ![](./figures/system_tuning/jump_to_stream_1.png "跳转到对应流")

    **图 21** 跳转到对应流效果图<a id="跳转到对应流效果图"></a>

    ![](./figures/system_tuning/jump_to_stream_2.png "跳转到对应流效果图")
...
- 显示全部已隐藏的泳道
    ...
    **图 2** 显示隐藏的泳道<a id="显示隐藏的泳道"></a>

    ![](./figures/system_tuning/show_hidden_lanes_1.png "显示隐藏的泳道")

**Python调用栈泳道独立展示**

为方便详细定位问题代码位置，Python调用栈独立泳道呈现，如[**图 3** Python调用栈](#Python调用栈)所示。

**图 3** Python调用栈<a id="Python调用栈"></a>
...
- 算子MFU分析
  ...
  算子 MFU 分析是通过 Profiling 数据分析算子的模型 FLOPs 利用率（Model FLOPs Utilization，MFU），帮助识别核心计算算子是否充分利用芯片理论算力的功能。数据由 msprof-analyze 生成，展示cluster_analysis.db的OperatorMFU表，如[**图 13** 算子 MFU 分析](#算子MFU分析)所示，字段说明见[**表 12** 算子 MFU 分析字段说明](#算子MFU分析字段说明)。

  **图 13** 算子 MFU 分析<a id="算子MFU分析"></a>

  ![](./figures/system_tuning/operator_mfu_analysis_1.png "算子MFU分析")

  **表 12** 算子 MFU 分析字段说明<a id="算子MFU分析字段说明"></a>

  |中文字段|英文字段|说明|
  |--|--|--|
  |卡序号|Rank ID|Rank ID。|
  |算子名称|Operator Name|框架侧算子名称。|
  |Kernel 名称|Kernel Name|Device 侧 Kernel 名称。|
  |Kernel 开始时间（ns）|Kernel Start Time(ns)|Kernel 开始时间，单位ns。|
  |Kernel 结束时间（ns）|Kernel End Time(ns)|Kernel 结束时间，单位ns。|
  |Kernel 执行时长（ns）|Kernel Duration(ns)|Kernel 执行时长，单位ns。|
  |MFU（比值）|MFU(ratio)|MFU 比值。|
  |实际算力（TFLOPS）|Actual TFLOPS|按当前 Kernel 时长计算的实际 TFLOPS。|
  |芯片峰值算力（TFLOPS）|Chip Peak TFLOPS|按 Kernel 输入数据类型匹配到的芯片理论峰值，单位 TFLOPS。|
  |浮点运算量（FLOPs）|FLOPs|采集侧记录的算子 FLOPs。|
  |FLOPs 算子名称|FLOPs Operator Name|采集侧记录 FLOPs 时对应的算子名称。|
  |输入 Shape|Input Shape|Kernel 输入 Shape。|
  |输出 Shape|Output Shape|Kernel 输出 Shape。|
...
- 并行策略概览包括并行策略设置、并行策略图展示和专家建议，如[**图 2** 并行策略概览](#并行策略概览)所示，并行策略参数解释如[**表 1** 并行策略参数说明](#并行策略参数说明)所示。当设置好并行策略后，可分别选择“DP + PP”、“DP + PP + CP”、“DP + PP + TP”或“DP + PP + CP + TP”并行维度展示并行策略图，选择图中目标编号，可悬浮显示该目标编号的详情信息，进行查看；也可单击鼠标右键弹出菜单，单击“复制属性”，复制当前目标编号的详情信息，粘贴至本地进行查看分析。
    ...
    **表 1** 并行策略参数说明<a id="并行策略参数说明"></a>
    ...
    |性能指标|Performance Metric|可按照性能指标展示并行策略图，可选的性能指标参数根据选择的域维度而变化。<br> 当选择“无”时，表示不展示性能指标，即并行策略图上的卡片信息为默认状态。<br> 当选择其余参数时，会在选框后显示该参数的“筛选范围 (μs)”和色条，筛选范围值为该参数值的最小值和最大值，且并行策略图上的卡片会按照对应值被渲染填色，可直观地查看各卡性能情况。|
...
当选择“Dump”时，展示MoE模型专家负载均衡热力图，其是基于Dump的热力图，统计的每个MoE层中每一个专家处理的token数量。可选择“Dump-均衡前”或“Dump-均衡后”，单击![](./figures/system_tuning/zh-cn_image_0000002532040275.png)按钮导入对应文件，展示MoE模型专家负载均衡热力图，如[**图 7** MoE大模型专家负载均衡分析](#MoE大模型专家负载均衡分析)所示，参数解释如[**表 5** MoE大模型专家负载均衡分析参数说明](#MoE大模型专家负载均衡分析参数说明)所示。“Dump-均衡前”和“Dump-均衡后”的数据文件采集方式可参见《MindIE LLM开发指南》的“特性介绍 \> [负载均衡](https://www.hiascend.com/document/detail/zh/mindie/latest/mindiellm/llmdev/user_guide/feature/expert_parallelism_load_balancer.md)”章节。
...
- DP + PP维度
    ...
    当选择“DP + PP”并行维度时，可勾选“流水线并行”和“数据并行”，单击策略图中的选框，计算/通信概览柱状图随之而变动；根据需求选择性能指标，策略图会被渲染填色，便于直观地分析指标，如[**图 4** DP + PP维度](#DP-+-PP维度)所示。可设置性能指标对应的筛选范围，在目标编号中输入所需的编号，可精准定位目标。
...
- DP + PP + CP维度
    ...
    当“算法”选择“Megatron-LM\(tp-cp-ep-dp-pp\)”、“Megatron-LM\(tp-cp-pp-ep-dp\)”或“MindSpeed\(tp-cp-ep-dp-pp\)”时，会展示“DP + PP + CP”并行维度，可勾选“流水线并行”、“上下文并行”和“数据并行”，单击策略图中的选框，计算/通信概览柱状图随之而变动；根据需求选择性能指标，策略图会被渲染填色，便于直观地分析指标，如[**图 5** DP + PP + CP维度](#DP-+-PP-+-CP维度)所示。可设置性能指标对应的筛选范围，在目标编号中输入所需的编号，可精准定位目标。
...
- DP + PP + TP维度
    ...
    当“算法”选择“MindIE-LLM\(tp-dp-ep-pp-moetp\)”或“vLLM\(tp-pp-dp-ep\)”时，会展示“DP + PP + TP”并行维度，可勾选“流水线并行”、“张量并行”、“数据并行”和“专家并行”，单击策略图中的选框，计算/通信概览柱状图随之而变动；根据需求选择性能指标，策略图会被渲染填色，便于直观地分析指标，如[**图 6** DP + PP + TP维度](#DP-+-PP-+-TP维度)所示。可设置性能指标对应的筛选范围，在目标编号中输入所需的编号，可精准定位目标。
...
- DP + PP + CP + TP维度
    ...
    当“算法”选择“Megatron-LM\(tp-cp-ep-dp-pp\)”、“Megatron-LM\(tp-cp-pp-ep-dp\)”或“MindSpeed\(tp-cp-ep-dp-pp\)”时，会展示“DP + PP + CP + TP”并行维度，可勾选“流水线并行”、“张量并行”、“上下文并行”和“数据并行”，单击策略图中的选框，计算/通信概览柱状图随之而变动；根据需求选择性能指标，策略图会被渲染填色，便于直观地分析指标，如[**图 7** DP + PP + CP + TP维度](#DP-+-PP-+-CP-+-TP维度)所示。可设置性能指标对应的筛选范围，在目标编号中输入所需的编号，可精准定位目标。
...
单击通信算子缩略图后，可使用以下快捷键。选中状态以圆角描边标识，标题后的问号提供操作说明。

|快捷键|操作|
|--|--|
|W / S|放大 / 缩小时间轴。|
|A / D|向左 / 向右移动当前可见时间范围。|
|Shift + W / Shift + S|放大 / 缩小 Rank 轴，显示更少 / 更多 Rank。|
|上 / 下方向键|按完整 Rank 行上下移动。|
|Esc|退出图表键盘操作。|

缩放以绘图区内的鼠标位置为中心；鼠标不在绘图区时，以视窗中心缩放。支持长按连续操作。时间轴每次按 1.2 倍缩放，平移步长为当前可见范围的 10%。单击右上方的重置缩放图标，可恢复完整时间范围和全部 Rank；全量视图下该图标置灰。
...
**图 3** 跳转至通信算子缩略图<a id="跳转至通信算子缩略图"></a>
