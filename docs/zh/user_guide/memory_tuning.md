...
内存块生命周期图的最大放大倍率由视口大小和数据坐标精度决定。达到上限后继续放大会保持当前视图，仍可缩小或重置；在缩略轴上选择过小的时间区间时，会自动扩展到可稳定显示的范围，并同步更新关联视图。
...
- 区域一：如[**图 1**  内存快照内存块生命周期图](#内存快照内存块生命周期图)所示。包括分窗栏和内存块生命周期图。其中分窗栏用于做数据分窗使用，内存块生命周期图展示reserved折线图、内存申请/释放折线图和内存块图。选择内存块图上的色块，展示该内存块的详情，并统计缩放条选中区间内的**区间未释放总量**、**区间未释放最大块**以及**区间未释放最小块**，以上三个统计量含义如下：
  ...
  > [!NOTE] 说明
  >
  >1. 数据分窗栏按固定事件数将每个 device 的快照拆分为多个分窗，当前默认每窗最多包含 500000 个原始 trace event。解析按事件 ID 从右向左倒序生成分窗，使最接近快照时刻的窗口优先可用。
  >2. reserved折线图：PTA缓存池总大小（Operator Reserved）情况的趋势图，表示PTA Caching缓存池总大小的变化趋势，直观呈现实际PTA从驱动处申请及管理的缓存池大小，为任意时刻所有segment大小的总和。
...
通过点击分窗栏选择不同的分窗，显示对应分窗下的数据。选择分窗后，内存快照内存块生命周期图、内存快照内存池状态图、内存快照详情数据均展示该分窗下对应的数据，具体如[**图 1** 分窗栏选择](#分窗栏选择)所示。

**图 1**  分窗栏选择<a id="分窗栏选择"></a>

![](./figures/memory_tuning/windows_select_1.png "分窗栏选择")

**内存块生命周期图支持查看指定时间范围内数据**

MindStudio Insight 内存块生命周期图通过控制下方内存快照趋势图缩放条选择区域展示指定范围内数据，如[**图 2**  下方内存快照趋势图缩放条](#内存快照趋势图缩放条)所示。

**图 2**  内存快照趋势图缩放条<a id="内存快照趋势图缩放条"></a>
...
2. 时间域选择多样化操作：
    ...
    - 可选中整个趋势图范围内的任意范围以选中对应时间区间，并展示对应内存情况。
    - 可固定时间长度，拖拽左右平移查看固定时间长度下的对应内存情况。
    - 可通过趋势图缩放条选择不同分窗，展示不同分窗下的数据。

**支持标记差值对比功能**

鼠标悬停生命周期图内存块后点击`K`键或直接点击标记轴，完成内存块标记和取消标记，此时会在图右侧标记轴上展示或取消展示该标记，可同时标记多个内存块，并展示标记轴相邻两个标记的内存差值，如[**图 3** 差值比对](#差值比对)图例所示。

**图 3**  差值比对<a id="差值比对"></a>

![](./figures/memory_tuning/comparative_difference_1.png "差值比对")

**生命周期图工具条使用说明**

- **缩放视图**：通过工具条切换等比或横向模式。滚轮按当前模式缩放；`W`键和`S`键横向缩放，`Ctrl`键 + `W`键/`S`键等比缩放。
- **平移视图**：拖拽图表完成平移，也可通过`A`键/`D`键或者方向键平移视图。
...
具体介绍也可通过界面右侧工具栏触发，如[**图 4** 生命周期图工具条指引](#生命周期图工具条指引)图例所示。

**图 4**  生命周期图工具条指引<a id="生命周期图工具条指引"></a>

![](./figures/memory_tuning/timeline_graph_tools_1.png "生命周期图工具条指引")

<a id="内存池状态图展示"></a>

**内存池状态图展示**

当鼠标置于“内存块生命周期图”中，在“内存块生命周期图”区域，单击内存色块，则会在“内存块生命周期图”下方展示对应时间点的事件总览以及内存池状态图。也可以通过搜索对应的地址精准定位事件详情，支持按照具体事件类型筛选数据，如[**图 5** 内存池状态图](#内存池状态图)所示。

> [!NOTE] 说明
>
>若点击内存色块后内存池状态未更新，则说明该内存色块在生命周期中未被采集到申请事件。

**图 5**  内存池状态图<a id="内存池状态图"></a>
...
- 内存块视图：展示缩放条选中区间内存块的详细信息，如[**图 6**  快照内存块视图](#快照内存块视图)所示，字段解释如[**表 1**  快照内存块视图字段说明](#快照内存块视图字段说明)所示。鼠标悬浮在视图中，右上角会浮现![](./figures/system_tuning/zh-cn_image_0000002500040346.png)按钮，单击复制表中当前所展示的内容。

    **图 6**  快照内存块视图<a id="快照内存块视图"></a>
  ...
  内存块视图提供“**自动筛选区间未释放**”功能，该功能的筛选逻辑如下：
    ...
    - 未勾选该功能，展示所有申请释放区间与缩放条选中区间有交集的内存块详情，具体示意图如[**图 7** 自动筛选区间未释放展示逻辑示意图](#自动筛选区间未释放展示逻辑示意图)中“开启前”所示，实际效果图如[**图 8** 未勾选自动筛选区间未释放内存块视图](#未勾选自动筛选区间未释放内存块视图)。

    - 勾选该功能，展示所有在缩放条选中区间内存块申请但未释放的内存块详情；用于识别潜在的泄露 tensor 占用内存块，具体示意图如[**图 7** 自动筛选区间未释放展示逻辑示意图](#自动筛选区间未释放展示逻辑示意图)中“开启后”所示，实际效果图如[**图 9** 勾选自动筛选区间未释放内存块视图](#勾选自动筛选区间未释放内存块视图)。

      **图 7**  自动筛选区间未释放展示逻辑示意图<a id="自动筛选区间未释放展示逻辑示意图"></a>

      ![](./figures/memory_tuning/potential_leak_tensor_filter_rule.png "自动筛选区间未释放展示逻辑示意图")

      **图 8**  未勾选自动筛选区间未释放内存块视图<a id="未勾选自动筛选区间未释放内存块视图"></a>

      ![](./figures/memory_tuning/memory_pytorch_snapshot_block_view_unselected_2.png "未勾选筛选内存块视图")

      **图 9**  勾选自动筛选区间未释放内存块视图<a id="勾选自动筛选区间未释放内存块视图"></a>

      ![](./figures/memory_tuning/memory_pytorch_snapshot_block_view_selected_2.png "勾选自动筛选区间未释放内存块视图")

- 内存事件视图：展示缩放条选中区域内存事件的详细信息，如[**图 10**  快照内存事件视图](#快照内存事件视图)所示，字段解释如[**表 2**  快照内存事件视图字段说明](#快照内存事件视图字段说明)所示。鼠标悬浮在视图中，右上角会浮现![](./figures/system_tuning/zh-cn_image_0000002500040346.png)按钮，单击复制表中当前所展示的内容。

    当在“内存块生命周期图”中选择不同“设备ID”时，内存事件视图的展示信息也会随之更新；当框选“内存块生命周期图”中部分区域展示时，内存事件视图的信息也会随之更新，展示的是框选时间范围内的所有内存事件。

    **图 10**  快照内存事件视图<a id="快照内存事件视图"></a>
...
- 选中详情：支持查看多个详情信息，可通过点击内存块生命周期图中的色块，内存池状态图左侧事件列表的事件以及内存池状态图中的色块查看详情信息。详情信息主要分为以下三类，包括：
  ...
  - Block类型选中详情，如[**图 12**  Block类型选中详情](#Block类型选中详情)所示，字段解释如[**表 4**  Block类型选中详情字段说明](#Block类型选中详情字段说明)所示，基础事件信息在左侧区域呈现，具体事件信息可切换事件类型查看。

    **图 12**  Block类型选中详情<a id="Block类型选中详情"></a>
  ...
  - segment类型选中详情，如[**图 13**  segment类型选中详情](#segment类型选中详情)所示，字段解释如[**表 5**  segment类型选中详情字段说明](#segment类型选中详情字段说明)所示，基础事件信息在左侧区域呈现，具体事件信息可切换事件类型查看。

    **图 13**  segment类型选中详情<a id="segment类型选中详情"></a>
