...
- 内存块视图：展示缩放条选中区间内存块的详细信息，如[**图 3**  快照内存块视图](#快照内存块视图)所示，字段解释如[**表 1**  快照内存块视图字段说明](#快照内存块视图字段说明)所示。鼠标悬浮在视图中，右上角会浮现![](./figures/system_tuning/zh-cn_image_0000002500040346.png)按钮，单击复制表中当前所展示的内容。
  ...
  内存块视图提供“**自动筛选区间未释放**”功能，该功能的筛选逻辑如下：
    ...
    - 未勾选该功能，展示所有申请释放区间与缩放条选中区间有交集的内存块详情，具体示意图如[图 4 自动筛选区间未释放展示逻辑示意图](#自动筛选区间未释放展示逻辑示意图)中“开启前”所示，实际效果图如[图 5 未勾选自动筛选区间未释放内存块视图](#未勾选自动筛选区间未释放内存块视图)。

    - 勾选该功能，展示所有在缩放条选中区间内存块申请但未释放的内存块详情；用于识别潜在的泄露 tensor 占用内存块，具体示意图如[图 4 自动筛选区间未释放展示逻辑示意图](#自动筛选区间未释放展示逻辑示意图)中“开启后”所示，实际效果图如[图 6 勾选自动筛选区间未释放内存块视图](#勾选自动筛选区间未释放内存块视图)。

      **图 4**  自动筛选区间未释放展示逻辑示意图<a id="自动筛选区间未释放展示逻辑示意图"></a>
      ![](./figures/memory_tuning/potential_leak_tensor_filter_rule.png "自动筛选区间未释放展示逻辑示意图")

      **图 5**  未勾选自动筛选区间未释放内存块视图<a id="未勾选自动筛选区间未释放内存块视图"></a>
      ![](./figures/memory_tuning/memory_pytorch_snapshot_block_view_unselected_2.png "未勾选筛选内存块视图")

      **图 6**  勾选自动筛选区间未释放内存块视图<a id="勾选自动筛选区间未释放内存块视图"></a>
