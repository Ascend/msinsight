...
鼠标悬停生命周期图内存块后点击`K`键或直接点击标记轴，完成内存块标记和取消标记，此时会在图右侧标记轴上展示或取消展示该标记，可同时标记多个内存块，并展示标记轴相邻两个标记的内存差值，如[**图2** 差值比对](#差值比对)图例所示。

**图 2**  差值比对<a id="差值比对"></a>
...
**图 3**  生命周期图工具条指引<a id="生命周期图工具条指引"></a>
...
当鼠标置于“内存块生命周期图”中，在“内存块生命周期图”区域，单击内存色块，则会在“内存块生命周期图”下方展示对应时间点的事件总览以及内存池状态图。也可以通过搜索对应的地址精准定位事件详情，支持按照具体事件类型筛选数据，如[**图 4** 内存池状态图](#内存池状态图)所示。

> [!NOTE] 说明
>
>若点击内存色块后内存池状态未更新，则说明该内存色块在生命周期中未被采集到申请事件。

**图 4**  内存池状态图<a id="内存池状态图"></a>
...
- 内存块视图：展示缩放条选中区间内存块的详细信息，如[**图 5**  快照内存块视图](#快照内存块视图)所示，字段解释如[**表 1**  快照内存块视图字段说明](#快照内存块视图字段说明)所示。鼠标悬浮在视图中，右上角会浮现![](./figures/system_tuning/zh-cn_image_0000002500040346.png)按钮，单击复制表中当前所展示的内容。
  ...
  内存块视图提供“**自动筛选区间未释放**”功能，该功能的筛选逻辑如下：
    ...
    - 未勾选该功能，展示所有申请释放区间与缩放条选中区间有交集的内存块详情，具体示意图如[图 6 自动筛选区间未释放展示逻辑示意图](#自动筛选区间未释放展示逻辑示意图)中“开启前”所示，实际效果图如[图 7 未勾选自动筛选区间未释放内存块视图](#未勾选自动筛选区间未释放内存块视图)。

    - 勾选该功能，展示所有在缩放条选中区间内存块申请但未释放的内存块详情；用于识别潜在的泄露 tensor 占用内存块，具体示意图如[图 6 自动筛选区间未释放展示逻辑示意图](#自动筛选区间未释放展示逻辑示意图)中“开启后”所示，实际效果图如[图 8 勾选自动筛选区间未释放内存块视图](#勾选自动筛选区间未释放内存块视图)。

      **图 6**  自动筛选区间未释放展示逻辑示意图<a id="自动筛选区间未释放展示逻辑示意图"></a>

      ![](./figures/memory_tuning/potential_leak_tensor_filter_rule.png "自动筛选区间未释放展示逻辑示意图")

      **图 7**  未勾选自动筛选区间未释放内存块视图<a id="未勾选自动筛选区间未释放内存块视图"></a>

      ![](./figures/memory_tuning/memory_pytorch_snapshot_block_view_unselected_2.png "未勾选筛选内存块视图")

      **图 8**  勾选自动筛选区间未释放内存块视图<a id="勾选自动筛选区间未释放内存块视图"></a>

      ![](./figures/memory_tuning/memory_pytorch_snapshot_block_view_selected_2.png "勾选自动筛选区间未释放内存块视图")

- 内存事件视图：展示缩放条选中区域内存事件的详细信息，如[**图 9**  快照内存事件视图](#快照内存事件视图)所示，字段解释如[**表 2**  快照内存事件视图字段说明](#快照内存事件视图字段说明)所示。鼠标悬浮在视图中，右上角会浮现![](./figures/system_tuning/zh-cn_image_0000002500040346.png)按钮，单击复制表中当前所展示的内容。

    当在“内存块生命周期图”中选择不同“设备ID”时，内存事件视图的展示信息也会随之更新；当框选“内存块生命周期图”中部分区域展示时，内存事件视图的信息也会随之更新，展示的是框选时间范围内的所有内存事件。

    **图 9**  快照内存事件视图<a id="快照内存事件视图"></a>
...
- 选中详情：支持查看多个详情信息，可通过点击内存块生命周期图中的色块，内存池状态图左侧事件列表的事件以及内存池状态图中的色块查看详情信息。详情信息主要分为以下三类，包括：
  - 事件类型选中详情，如[**图 10**  事件类型选中详情](#事件类型选中详情)所示，字段解释如[**表 3**  事件类型选中详情字段说明](#事件类型选中详情字段说明)所示，基础事件信息在左侧区域呈现，具体事件信息可切换事件类型查看。

    当在“内存块生命周期图”中单击任意时间点的视图，快照选中详情的展示信息也会随之更新。

    **图 10**  事件类型选中详情<a id="事件类型选中详情"></a>
  ...
  - Block类型选中详情，如[**图 11**  Block类型选中详情](#Block类型选中详情)所示，字段解释如[**表 4**  Block类型选中详情字段说明](#Block类型选中详情字段说明)所示，基础事件信息在左侧区域呈现，具体事件信息可切换事件类型查看。
    ...
    **图 11**  Block类型选中详情<a id="Block类型选中详情"></a>
  ...
  - segment类型选中详情，如[**图 12**  segment类型选中详情](#segment类型选中详情)所示，字段解释如[**表 5**  segment类型选中详情字段说明](#segment类型选中详情字段说明)所示，基础事件信息在左侧区域呈现，具体事件信息可切换事件类型查看。
    ...
    **图 12**  segment类型选中详情<a id="segment类型选中详情"></a>
