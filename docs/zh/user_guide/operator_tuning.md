...
- 仅Ascend 950PR&950DT系列产品支持通过“msopprof”方式采集指令流水图可视化呈现文件，数据展示在时间线（Timeline）界面。其他硬件型号可通过“msopprof simulator”仿真模式采集trace.json文件，在时间线界面查看指令流水。
...
|CUBE|矩阵乘运算单元。|
|FIXP|数据搬运流水，数据搬运方向为：FIXPIPE L0C -> OUT/L1。<br> 仅Atlas A2系列产品导出的性能数据支持展示。|
...
- 区域三：指令表，查看指令记录，包括地址、内容、数量、次数等，表中字段解释如[**表 2**  指令表字段说明](#指令表字段说明)所示。
    ...
    **表 2**  指令表字段说明<a id="指令表字段说明"></a>
    ...
    |寄存器状态|GPR Status|寄存器依赖信息，以图形化展示。由多列带箭头的直线组成，每一列代表一个寄存器。其中左实心箭头表示为写，右空心箭头为读，竖线表示当前寄存器仍在使用。当鼠标悬停在寄存器上时，会显示寄存器信息。<br> 仅Ascend 950PR&950DT系列产品导出的数据支持显示该参数。|-|
...
> [!NOTE]
>
> - Atlas A3系列产品、Atlas A2系列产品和Ascend 950PR&950DT系列产品导出的性能数据支持此模块。
...
>[!NOTE]
>
> - 仅Ascend 950PR&950DT系列产品、Atlas A3系列产品、Atlas A2系列产品和Atlas推理系列产品支持此模块。
> - 当导入Ascend 950PR&950DT系列产品的数据时，Roofline模型图上将展示指令类型，可以结合图中参数进行筛选，以便查看Roofline模型图。

- 当硬件产品为Ascend 950PR&950DT系列产品、Atlas A3系列产品、Atlas A2系列产品时，Roofline性能模型分析包含内存单元、内存通路以及搬运单元页签。
...
- 当硬件产品为Atlas推理系列产品时，仅存在内存单元内容，如[**图 6**  内存单元模型图](#内存单元模型图)所示，参数解释如[**表 7**  内存单元参数说明](#内存单元参数说明-1)所示。
