...
- 仅<term>Ascend 950 系列产品</term>支持通过“msopprof”方式采集指令流水图可视化呈现文件，数据展示在时间线（Timeline）界面。其他硬件型号可通过“msopprof simulator”仿真模式采集trace.json文件，在时间线界面查看指令流水。
...
- 区域三：指令表，查看指令记录，包括地址、内容、数量、次数等，表中字段解释如[**表 2**  指令表字段说明](#指令表字段说明)所示。
    ...
    **表 2**  指令表字段说明<a id="指令表字段说明"></a>
    ...
    |寄存器状态|GPR Status|寄存器依赖信息，以图形化展示。由多列带箭头的直线组成，每一列代表一个寄存器。其中左实心箭头表示为写，右空心箭头为读，竖线表示当前寄存器仍在使用。当鼠标悬停在寄存器上时，会显示寄存器信息。<br> 仅<term>Ascend 950 系列产品</term>导出的数据支持显示该参数。|-|
...
- 区域一：基础信息（Base Info），如[**图 1** 基础信息](#基础信息)所示。
- 区域二：核间负载分析（Core Occupancy），如[**图 2** 核间负载分析](#核间负载分析)所示。
- 区域三：Roofline瓶颈分析（Roofline），如[**图 3** Roofline瓶颈分析-内存单元](#内存单元)所示。
- 区域四：计算工作负载分析（Compute Workload Analysis），如[**图 4** 计算工作负载分析](#计算工作负载分析)所示。
- 区域五：Top Warp Stall Season，如[**图 5** Top Warp Stall Season](#TopWarpStallSeason)所示。
- 区域六：内存负载分析（Memory Workload Analysis），如[**图 6** 内存负载分析](#内存负载分析)所示。
...
> [!NOTE]
>
> - <term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>、<term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>和<term>Ascend 950 系列产品</term>导出的性能数据支持此模块。
...
>[!NOTE]
>
> - 仅<term>Ascend 950 系列产品</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>、<term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>和<term>Atlas 推理系列产品</term>支持此模块。
> - 当导入<term>Ascend 950 系列产品</term>的数据时，Roofline模型图上将展示指令类型，可以结合图中参数进行筛选，以便查看Roofline模型图。

- 当硬件产品为<term>Ascend 950 系列产品</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>、<term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>时，Roofline性能模型分析包含内存单元、内存通路以及搬运单元页签。
...
导入指定目录下的`memory_info.json`文件，出现片上内存（On-Chip Memory）界面，展示内存分布情况，如[**图 1**  片上内存-内存块图](#片上内存-内存块图)所示，点击内存块图中的色块，则会展现该内存块的内存详情以及内存分布，如[**图 2**  片上内存-选中详情](#片上内存-选中详情)以及[**图 3**  片上内存-内存分布](#片上内存-内存分布)所示。
...
导入指定目录下的`memory_info.json`文件，界面出现内存块图，如[**图 1**  片上内存-内存块图](#片上内存-内存块图)所示。基本信息将会在界面最上方展示，包括算子名称、内存类型、编译状态信息。可以通过切换内存类型查看对应的片上内存情况和编译状态。若编译失败，则会展示编译失败的错误信息，如[**图 4**  片上内存-基本信息](#片上内存-基本信息)所示。

点击[**图 1**  片上内存-内存块图](#片上内存-内存块图)中任意的内存块，则会展现该内存块的内存详情以及内存分布，如[**图 2**  片上内存-选中详情](#片上内存-选中详情)以及[**图 3**  片上内存-内存分布](#片上内存-内存分布)所示。

**图 1**  片上内存-内存块图<a id="片上内存-内存块图"></a>
![](./figures/operator_tuning/memory_block_graph_1.png "片上内存-内存块图")

**图 2**  片上内存-选中详情<a id="片上内存-选中详情"></a>
![](./figures/operator_tuning/slice_detail_1.png "片上内存-选中详情")

**图 3**  片上内存-内存分布<a id="片上内存-内存分布"></a>
![](./figures/operator_tuning/memory_layout_1.png "片上内存-内存分布")

**图 4**  片上内存-基本信息<a id="片上内存-基本信息"></a>
