...
MindStudio Insight supports three data import modes. This section describes how to import data. Different tuning scenarios support different data formats. Before importing, check whether the data source and file type match.

| Tuning Scenario | Common Import Data | Detailed Description |
| --- | --- | --- |
| System | `json`, `db`, `csv`, cluster analysis directory | See [System Tuning Data Description](./system_tuning.md#data-description). |
| Operator | `trace.json`, `visualize_data.bin` | See [Operator Tuning Data Description](./operator_tuning.md#data-description). |
| Memory | `memscope_dump_*.db`, `pickle` | See [Memory Tuning Data Description](./memory_tuning.md#data-description). |
| Tuning as a service | `profiler.db`, `chrome_tracing.json`, `ms_service_*.db` | See [Serving Tuning Data Description](./service_optimization.md#data-description). |
...
>[!NOTE]
>
> - Only local drive data can be imported. If the network drive is used, map the network drive to the local PC and then import the data to the corresponding directory. For details about how to map the network drive to the local PC, see [Data Cannot Be Loaded When a Network Drive Directory Is Dragged to MindStudio Insight](../support/faq.md#faq-network-disk-import).
> - If dragging a file into MindStudio Insight on a Windows system is not allowed, refer to [File Drag and Drop in MindStudio Insight Is Disabled](../support/faq.md#faq-drag-file-disabled) for resolution.

**Import Failure Troubleshooting**

| Symptom | Possible Cause | Suggestion |
| --- | --- | --- |
| The target tab is not displayed after import. | The imported data lacks the files required for the tab, or the collection switch is not enabled. | Check whether the data files are complete by referring to the data description in the corresponding tuning guide. |
| The page is empty after importing a directory. | An incorrect directory level was selected, or there are no parsable files in the directory. | Select the root directory containing the profile data files again, or specify the files. |
| Dragging a file has no effect. | Restricted by network disk access, system permissions, or drag‑and‑drop capabilities. | Use the "Select profile Data Path" or "Enter profile Data Path" method to import. |
| Importing a large file takes a long time. | The data volume is large or preprocessing is required. | Wait for parsing to complete. For large-scale cluster data, it is recommended to preprocess it with the corresponding tool before importing. |
| The cause still cannot be identified. | Logs or FAQs need to be checked. | For the log path, see [Managing Logs](#managing-logs). For common issues, see [FAQs](../support/faq.md). |
...
- Manual clearance
    ...
    Go to the log file storage path and manually delete the log files. For details about the log file storage paths, see [Viewing the Log File Paths](#log_file_paths).
...
This section describes the document viewing feature of the MindStudio Insight tool. If you encounter any issues, you can first click ![](./figures/basic_operations/en-us_image_0000002500056862.png) in the upper right corner of the MindStudio Insight tool interface, and select "Help Documentation" to view the documentation for this tool. If the issue persists, click "[Help and Feedback](#help-and-feedback)" to go to the community and submit an issue for assistance.

## Help and Feedback

This section describes the help and feedback feature of the MindStudio Insight tool. If you encounter unresolved issues or have constructive comments or suggestions, you can click ![](./figures/basic_operations/en-us_image_0000002500056862.png) in the upper right corner of the MindStudio Insight tool interface, select "Help and Feedback", and go to the community to seek assistance or submit feedback.
