...
- Credentials in Agent settings are wrapped by application-layer encryption before the browser submits them. This reduces the risk of plaintext credentials appearing in ordinary proxy or request logs. Because the public key is delivered over the same connection, this mechanism does not prevent an active man-in-the-middle attack and does not replace HTTPS. Remote JupyterLab and Docker deployments must use HTTPS and appropriate access controls.
- Persisted Agent credentials are encrypted with a separate wrapping key. For containers, set `MSINSIGHT_SECRETS_KEY_PATH` to a permission-restricted persistent mount or secret file and manage it separately from the configuration data. If ciphertext configuration is persisted without the original wrapping key, existing credentials cannot be decrypted after the container is replaced.
