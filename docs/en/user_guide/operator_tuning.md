...
Based on your tuning objectives, you can refer to the following paths to focus your reading:

- **Determine the operator bottleneck type**: First read the Roofline bottleneck analysis in [Details](#details) to determine whether it is a compute bottleneck or a memory bottleneck.
- **Analyze compute bottlenecks**: Read the compute workload analysis and inter-core load analysis in [Details](#details).
- **Analyze memory bottlenecks**: Read the memory load analysis in [Details](#details) and [Cache](#cache).
- **Locate high-latency instructions**: Read [Timeline](#timeline).
- **Correlate instructions to source code**: Read [Source](#source).
- **Analyze Triton operator memory overflow**: Read [On-Chip Memory](#on-chip-memory).
- **Compare performance before and after optimization**: Read "Supporting Inter-Operator Performance Comparison" in [Details](#details).

## Preparations

**Environment Setup**

1. Install MindStudio Insight first. For details, see [MindStudio Insight Installation Guide](../install_guide/mindstudio_insight_install_guide.md).
2. Install the msOpProf operator tuning tool. For detailed installation steps, see [msOpProf Installation Guide](https://gitcode.com/Ascend/msopprof/blob/master/docs/en/install_guide/msopprof_install_guide.md).

**Data Collection**

The data for the operator tuning scenario is collected and generated by the msOpProf tool. Select a collection mode based on your actual requirements:

| Collection Mode | Applicable Scenario | Collection Command Example | Generated File |
| -- | -- | -- | -- |
| On-Board Collection (msopprof) | Requires real hardware profile data to analyze actual runtime duration, memory bandwidth, cache behavior, etc. | `msopprof --output=./output_dir ./op_executable` | `visualize_data.bin` |
| Simulation Collection (msopprof simulator) | Requires no real hardware or instruction-level detailed analysis, such as instruction pipeline tracing, code hotspot locating, etc. | `msopprof simulator --soc-version=Ascendxxxyy --output=./output_dir ./op_executable` | `trace.json` and `visualize_data.bin` |

>[!NOTE]
>
> - On-board collection requires a real NPU environment. Simulation collection does not require an NPU but requires specifying the chip model (`--soc-version`), which can be obtained via `python3 -c "import acl; print(acl.get_soc_name())"`.
> - The two modes are complementary. It is recommended to use them together: use on-board collection to obtain real profile data, and simulation collection to obtain instruction-level detailed data.
> - For more collection parameters and usage, see the [msopprof User Guide](https://gitcode.com/Ascend/msopprof/blob/master/docs/en/user_guide/msopprof_user_guide.md) and the [msopprof simulator User Guide](https://gitcode.com/Ascend/msopprof/blob/master/docs/en/user_guide/msopprof_simulator_user_guide.md).

**Data Import**

After collection is complete, import the generated profile data files into the MindStudio Insight tool. For details about the data, see [Data Description](#data-description). For data import operations, see [Importing Data](./basic_operations.md#importing-data).
...
|Display Interface|File Name|Description|File Acquisition Method|
|--|--|--|--|
|Timeline|trace.json|Operator simulation instruction pipeline diagram file.|Simulation Mode only. See [msopprof Simulator Mode User Guide](https://gitcode.com/Ascend/msopprof/blob/master/docs/en/user_guide/msopprof_simulator_user_guide.md).|
|Timeline|visualize_data.bin|Instruction pipeline visualization file.|On-Board + Simulation Mode. See [msopprof Mode User Guide](https://gitcode.com/Ascend/msopprof/blob/master/docs/en/user_guide/msopprof_user_guide.md) and [msopprof Simulator Mode User Guide](https://gitcode.com/Ascend/msopprof/blob/master/docs/en/user_guide/msopprof_simulator_user_guide.md).|
|Details|visualize_data.bin|Visualization file for operator basic information, compute unit load, and Roofline bottleneck analysis.|On-Board Mode only. See [msopprof Mode User Guide](https://gitcode.com/Ascend/msopprof/blob/master/docs/en/user_guide/msopprof_user_guide.md).|
|Source|visualize_data.bin|Visualization file for simulation hotspot function information.|On-Board + Simulation Mode. See [msopprof Mode User Guide](https://gitcode.com/Ascend/msopprof/blob/master/docs/en/user_guide/msopprof_user_guide.md) and [msopprof Simulator Mode User Guide](https://gitcode.com/Ascend/msopprof/blob/master/docs/en/user_guide/msopprof_simulator_user_guide.md).|
|Cache|visualize_data.bin|Visualization file for L2 Cache access information within user program kernel functions.|On-Board Mode only. See [msopprof Mode User Guide](https://gitcode.com/Ascend/msopprof/blob/master/docs/en/user_guide/msopprof_user_guide.md).|

>[!NOTE]
>
> The same `visualize_data.bin` file can contain multiple types of visualization data simultaneously. After import, MindStudio Insight automatically identifies and displays them in the corresponding interfaces. The data content displayed in different interfaces depends on the msopprof mode and parameters used during collection.

**Constraints**

- In the operator tuning scenario, to import a JSON file into MindStudio Insight, the file must contain the "profilingType":"op" field before the first square bracket. Otherwise, the file cannot be imported. JSON files generated by the msOpProf tool include this field by default. If the import fails, check whether the JSON file content contains this field.
...
- Only the Atlas 350 accelerator card supports the collection of instruction pipeline diagrams using the msopprof method and visualized display of the collected data on the timeline page. For other hardware models, you can collect `trace.json` files via the "msopprof simulator" simulation mode to view the instruction pipeline in the **Timeline** interface.

**Terminology**

|Term|Full Name|Description|
|--|--|--|
|HBM|High Bandwidth Memory|The external main memory of the AI processor, featuring large capacity but high access latency.|
|GM|Global Memory|HBM, the global storage space accessible to operators.|
|L2|L2 Cache|Located between HBM and AI Core, used to cache frequently accessed data to reduce access latency.|
|L1|L1 Buffer|The storage space inside the AI Core, shared by the Cube and Vector Compute Units.|
|L0A/L0B|L0A/L0B Buffer|Input buffers of the Cube Compute Unit, storing the left and right matrices of matrix multiplication, respectively.|
|L0C|L0C Buffer|Output buffer of the Cube Compute Unit, storing the result of matrix multiplication.|
|UB|Unified Buffer|The dedicated storage space of the Vector Compute Unit, used for data input and output of vector operations.|
|MTE1|Memory Transfer Engine 1|Transfer direction: L1 → {L0A, L0B, UB}.|
|MTE2|Memory Transfer Engine 2|Transfer direction: {DDR/GM, L2} → {L1, L0A/B, UB}.|
|MTE3|Memory Transfer Engine 3|Transfer direction: UB → {DDR/GM, L2, L1}, L1 → {DDR/L2}.|
|FIXP|Fix Pipeline|Data transfer pipeline, transfer direction: L0C → OUT/L1.|
|Pipe|Pipeline|The execution channels for different types of instructions in the AI Core.|
|Core|AI Core|The compute core in the AI processor, containing compute units such as Cube and Vector.|
|Block|Sub Block|Parallel blocks during operator execution, with each block running on one AI Core.|
...
| MTE1           | Data transfer pipeline, from L1 to {L0A/L0B, UB}.                                                                                                                                                                                                                                                                                                                                                 |
...
| MTE2           | Data transfer pipeline, from {DDR/GM, L2} to {L1, L0A/B, UB}.                                                                                                                                                                                                                                                                                                                                     |
| VECTOR         | Vector unit.                                                                                                                                                                                                                                                                                                                                                                                        |
| MTE3           | Data transfer pipeline, from UB to {DDR/GM, L2, L1}, or from L1 to {DDR/L2}.                                                                                                                                                                                                                                                                                                                      |
...
The **Timeline** interface supports adjusting the view through the mouse function bar. The interface is shown in [Figure 1 Floating Toolbar](#floating-toolbar), and the main functions are as follows:

1. Default mode: You can flexibly click with the mouse and freely select units by dragging a box.
2. Pan mode: Freely drag the timeline chart horizontally to flexibly view and locate operators.
3. Auto fit lane height : Automatically compresses blank and invalid areas in response to window resizing, making the view display clearer.
4. Floating toolbar guide: Provides an illustrated introduction to the floating toolbar functions.
5. The floating toolbar can be hidden and shown via key press.

    **Figure 1**  Floating toolbar<a id="floating-toolbar"></a>
...
The **Details** tab page displays **Base Info**, **Compute Workload Analysis**, **Core Occupancy**, **Roofline**, **Top Warp Stall Season**, and **Memory Workload Analysis**. The analysis results are displayed in charts and data panes.

### GUI Description

The **Details** tab page consists of six areas:

- Area 1: **Base Info**, as shown in [**Figure 1** Base Info](#base-info).
- Area 2:  **Core Occupancy**, as shown in [**Figure 2** Core Occupancy](#core-occupancy).
- Area 3: **Roofline**, as shown in [**Figure 3** Roofline - Memory Unit](#memory-unit).
- Area 4: **Compute Workload Analysis**, as shown in [**Figure 4** Compute Workload Analysis](#compute-workload-analysis).
- Area 5: **Top Warp Stall Season**, as shown in [**Figure 8** Top Warp Stall Season](#TopWarpStallSeason).
- Area 6: **Memory Workload Analysis**, as shown in [**Figure 9** Memory Workload Analysis](#memory-workload-analysis)
...
**Figure 1**  Base Info<a id="base-info"></a>

![](./figures/operator_tuning/basic_info_1.png)
...
| Op Type          | Operator type. The options are **mix**, **vector**, **cube**, and **AI Core**.                                                                                                                        |
| Device Id        | Device ID.                                                                                                                                                                                           |
| Pid              | Process ID                                                                                                                                                                                           |
| Block Dim        | Number of sub blocks. This parameter is used when the operator type is **vector**, **cube**, or **AI Core**.                                                                                          |
...
| Field         | Description                                                                         |
| ------------- | ----------------------------------------------------------------------------------- |
| Block ID      | Sub block ID. This parameter is not available when the operator type is **AI Core**. |
...
>[!NOTE]
>
> - This module is supported by the profile data exported from the <term>Atlas A3 training products/Atlas A3 inference products</term>, <term>Atlas A2 training products/Atlas A2 inference products</term>, and Ascend 950 products.
...
>[!NOTE]
>
> - This module is supported only by the Ascend 950 products, <term>Atlas A3 training products/Atlas A3 inference products</term>, <term>Atlas A2 training products/Atlas A2 inference products</term>, and <term>Atlas inference products</term>.
> - When the data of the Ascend 950 products is imported, the instruction types are displayed in the Roofline model graph. You can filter the instruction types based on the parameters in the graph to view the Roofline model graph.

- For the Ascend 950 products, <term>Atlas A3 training products/Atlas A3 inference products</term>, and <term>Atlas A2 training products/Atlas A2 inference products</term>, the Roofline model analysis includes the memory unit, memory channel, and MTE tabs.
...
| Parameter         | Description                                                                                                                                                                                                                                                                                                                                               |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Block ID          | Sub block ID. You can switch the block ID to view the corresponding information. When the operator type is **AI Core**, this parameter is displayed as **NA**, and the multi-core average value is displayed.                                                                                                                                              |
...
| AICORE            | Name of an AI Core instruction. This parameter is displayed when the operator type is **AI Core**.                                                                                                                                                                                                                                                         |
...
Area 5 is the Top Warp Stall Season chart, which mainly displays the wait counts caused by resource conflicts, data dependencies, and other reasons during instruction execution, including active. See [**Figure 8**  Top Warp Stall Season](#TopWarpStallSeason) for details, and see [**Table 9**  Top Warp Stall Season parameter description](#TopWarpStallSeason-parameter-description) for parameter descriptions.

**Figure 8** Top Warp Stall Season<a id="TopWarpStallSeason"></a>

![](./figures/operator_tuning/top_warp_stall_season_1.png "Top Warp Stall Season")

**Table 9** Top Warp Stall Season parameter description<a id="TopWarpStallSeason-parameter-description"></a>

|Parameter|Description|
|--|--|
|Nop_Cycles|No-operation cycles.|
|Divergence_Stack_Spill|Branch divergence stack spill. Pipeline stall cycles caused by insufficient stack space or mismatch between predicted and actual paths during branch instruction execution.|
|IBuf_Empty|Instruction buffer empty. Pipeline stall cycles caused by the inability to fetch the next instruction when IBuf is empty.|
|Register_bank_conflict|Register bank conflict. Pipeline stall cycles caused by conflicts when instructions access the same register bank, forcing the pipeline to pause to avoid data errors.|
|Resource_conflict|Resource conflict. Pipeline stall cycles caused by multiple instructions simultaneously competing for the same hardware resource and being unable to execute in parallel.|
|ScoreBoard_Not_Ready|Scoreboard not ready. Pipeline stall caused by the ScoreBoard state not yet being ready (e.g., preceding instructions still occupying relevant resources or registers), preventing the current instruction from continuing execution.|
|Warp_Level_Sync|Warp-level synchronization wait. In multi-threaded execution, instruction execution is stalled to ensure synchronization and consistency among threads within the same warp when synchronization conditions are not met.|
|active|Number of times instructions are successfully issued without stalls.|
|Others|Pipeline stall cycles triggered by other exceptions or control logic beyond the explicitly classified stall causes.|

**Memory Workload Analysis**

Area 6: **Memory Workload Analysis**. It displays the memory workload analysis information in the memory heatmap and data pane, as shown in [**Figure 9** Memory Workload Analysis](#memory-workload-analysis). The parameters in the memory heatmap and data pane are displayed based on the collected data. For details about the parameters, see [Table 10 Parameter description](#parameter-description). **Peak\(%\)** on the right of the heatmap indicates the arrow color, with the value representing the peak bandwidth proportion (i.e., the proportion of the maximum bandwidth). The content indicated by the ![](./figures/operator_tuning/en-us_image_0000002532040351.png) icon indicates the memory workload analysis result of each block.

**Figure 9** Memory Workload Analysis <a id="memory-workload-analysis"></a>

![](./figures/operator_tuning/memory_load_analysis_1.png "Memory Workload Analysis")

**Table 10** Parameter description<a id="parameter-description"></a>

| Parameter | Description                                                                                                                                                                                                             |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Block ID  | Sub block ID. You can select the sub block to be viewed from the **Block ID** drop-down list. When the operator type is **AI Core**, **Block ID** is displayed as **NA**, and the multi-core average value is displayed. |
...
- [**Figure 3** On-Chip Memory - Slice Detail](#on-chip-memory-slice-detail) shows the physical layout of the memory, helping you tune the memory and reduce bank conflicts. To understand the layout of this figure, you need to understand the memory layout of the Ascend AI Processor. For details, see "[Avoiding Bank Conflicts in the Unified Buffer](https://www.hiascend.com/document/detail/en/CANNCommunityEdition/910/programug/Ascendcopdevg/docs/guide/%E7%AE%97%E5%AD%90%E5%AE%9E%E8%B7%B5%E5%8F%82%E8%80%83/SIMD%E7%AE%97%E5%AD%90%E6%80%A7%E8%83%BD%E4%BC%98%E5%8C%96/%E5%86%85%E5%AD%98%E8%AE%BF%E9%97%AE/%E9%81%BF%E5%85%8DUB%E7%9A%84bank%E5%86%B2%E7%AA%81/%E6%A6%82%E8%BF%B0.md)" in *Ascend C Operator Development Guide*.
