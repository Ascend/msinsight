...
<!-- md-trans-meta sourceCommit=49122e1f1d6621e5b8a8bbc1c0fdae9583659f48 translatedAt=2026-08-12T11:47:49.568Z pushedAt=2026-08-12T11:57:31.154Z -->
...
MindStudio Insight provides a rich set of tuning analysis methods, visually presents real software and hardware running data, analyzes performance bottlenecks from multiple dimensions, and supports visualized cluster performance analysis at scales of hundreds or thousands of ranks and above, helping developers improve performance tuning efficiency.

## Typical Usage Process

When using MindStudio Insight for performance analysis, follow the process below:

1. **Install the tool**: Complete the installation by following the [MindStudio Insight Installation Guide](../install_guide/mindstudio_insight_install_guide.md).

2. **Collect data**: Use msProf, msOpProf, msMemScope, or the serving profiling tool to collect profile data.

3. **Import data**: Import the corresponding file or directory into MindStudio Insight. For basic operations, see [MindStudio Insight Basic Operations](./basic_operations.md).

4. **Select a scenario**: Based on the issue type, enter the System Tuning, Operator Tuning, Service-oriented Tuning, or Memory Tuning interface.

5. **Locate bottlenecks**: Identify issues by combining the Timeline, detail tables, communication analysis, memory view, or source code view.

6. **Handle exceptions**: If the import fails or no data is displayed on the interface, first check the [FAQs](../support/faq.md).

## Scenario Selection Guide

| Issue Symptom | Recommended Entry | Data Collection Tool | Key Content to View |
| --- | --- | --- | --- |
| Slow overall model running, imbalance between ranks, abnormal communication duration | [System Tuning](./system_tuning.md) | msProf | Summary, Communication, Timeline, Host Bound analysis |
| High duration of a single operator, insufficient pipeline parallelism, obvious source code hotspots | [Operator Tuning](./operator_tuning.md) | msOpProf | Timeline, Source, Details, Cache |
| High inference request latency, long TTFT, abnormal KV Cache usage | [Serving Tuning](./service_optimization.md) | Serving Profiling | Timeline, Curve, request_data, batch_info, forward_info |
| Continuous memory growth, OOM, unreleased memory, or memory pool anomalies | [Memory Tuning](./memory_tuning.md) | msMemScope | MemScope memory details, PyTorch Snapshot, call stack flame graph |

## Related Resources

| Resource | Description |
| --- | --- |
| [MindStudio Insight Installation Guide](../install_guide/mindstudio_insight_install_guide.md) | Installation, upgrade, uninstallation, and integrity check. |
| [Release Notes](../release_notes/release_notes.md) | Version compatibility between MindStudio Insight and components such as CANN. |
| [FAQ](../support/faq.md) | Common issues regarding import, display, dependencies, network disks, and JupyterLab. |
| [Quick Start](../quick_start/system_tuning_quick_start.md) | Quickly experience the system tuning analysis path using sample data. |

## Advantages

- MindStudio Insight allows developers to view the profile data in the cluster scenario on the **Timeline** tab page, displays the data by single rank, and automatically traverses all .db, **trace_view.json** files (in PyTorch and MindSpore scenarios), and **msprof*.json** files (in TensorFlow and offline inference scenarios) in the input path. These files do not need to be manually merged.
...
- System tuning
  ...
  | Summary|Displays the computing and communication operator duration analysis, and the analysis results in a bar chart, curve, and data pane.|PyTorch cluster scenario is supported.|
  ...
  | Reinforcement Learning (RL)| Performs high-level abstraction based on collected data, visually presents the temporal relationships of control flows, helps quickly locate time-consuming tasks and bubbles, and supports further performance analysis.                     |-                                |
...
- Operator tuning
  ...
  | Details   | Displays the basic operator information, computing workload analysis, and memory workload analysis, as well as the analysis results in charts and data panes. | BIN files of operator profiling collected by msProf are supported.|
