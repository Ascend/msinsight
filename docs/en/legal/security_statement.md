...
- This tool is intended for development use only. It does not enforce ownership or permission restrictions on itself or on the files it processes. Users are responsible for assigning appropriate ownership and permissions based on their specific use cases, and for ensuring that all files processed by the tool are safe and trustworthy.
