 ...
 */
export function startNew(name, options = {}) {
...
export function getStaticConfig(settings) {
    const localSettings = settings || ServerConnection.makeSettings();
    const statisConfigUrl = Private.getMindStudioStaticConfigUrl(localSettings.baseUrl);
 ...
 */
export function listRunning(settings) {
    const localSettings = settings || ServerConnection.makeSettings();
    const serviceUrl = Private.getServiceUrl(localSettings.baseUrl);
    const instanceUrl = Private.getMindStudioInstanceRootUrl(localSettings.baseUrl);
    return ServerConnection.makeRequest(serviceUrl, {}, localSettings)
        ...
        const urls = Array.from(map(data, (item) => {
 ...
 */
export function shutdown(name, settings) {
    const localSettings = settings || ServerConnection.makeSettings();
    const url = Private.getMindStudioUrl(localSettings.baseUrl, name);
    const init = { method: 'DELETE' };
    return ServerConnection.makeRequest(url, init, localSettings).then((response) => {
 ...
 */
export function shutdownAll(settings) {
    const localSettings = settings || ServerConnection.makeSettings();
 ...
 */
export function startIframeUrl(settings) {
    const localSettings = settings || ServerConnection.makeSettings();
    ...
    return ServerConnection.makeRequest(iframConfigUrl, {}, localSettings)
        ...
        .then((data) => {
        return Private.getMindStudioInstanceUrl(localSettings.baseUrl, data.proxy, data.port, data.profilerServerId, data.acpPort, data.acpCapabilityToken);
    });
}
/**
 * Terminate mindstudio profiler server by iframe's url.
 */
export function terminateIframe(profilerServerId, settings) {
    const localSettings = settings || ServerConnection.makeSettings();
    const terminateProfilerUrl = Private.terminateIframe(localSettings.baseUrl, encodeURIComponent(profilerServerId));
    return ServerConnection.makeRequest(terminateProfilerUrl, {}, localSettings).then((response) => {
        if (response.status !== 200) {
            throw new ServerConnection.ResponseError(response);
        }
        return response.json();
...
//# sourceMappingURL=staticMindStudio.js.map